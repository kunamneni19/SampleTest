...
public class MainPageTest extends AbstractTestNGSpringContextTests {
    ...
    public void verifyProducts_defaultOrder(Product p) {
        ...
        softAssert.assertTrue(p.getName().matches(p.getName()), "The name " + p.getName() +" for product on position " + p.getPosition() +" is not the expected one");
        softAssert.assertTrue(p.getDescription().contains(p.getDescription()), "The description is not the expected one");
        softAssert.assertTrue(p.getPrice().contains(p.getPrice()), "The price is not the expected one");
    ...
    public void verifyProducts_sortByPrice() {
        ...
        System.out.println("products list is: "+product.getName());
        SoftAssert softAssert = new SoftAssert();
        softAssert.assertTrue(product.getName().matches(product.getName()), "The name is not the expected one");
        softAssert.assertTrue(product.getDescription().contains(product.getDescription()), "The description is not the expected one");
        softAssert.assertTrue(product.getPrice().contains(product.getPrice()), "The price is not the expected one");
        softAssert.assertAll();
    }
